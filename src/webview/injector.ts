export function buildInjector(card:any){
    return `
    (function(){
    ...
    async function autofill(){
     try{
      ...
      await waitFor('input[name="cardName"]')
      fillInput('input[name="cardName"]',"${card.name}")
      fillInput('input[name="cardNumber"]',"${card.number}")
      fillInput('input[name="cardDueDate"]',"${card.expiry}")
      fillInput('input[name="email"]',"${card.email}")
      if("${card.promo || ""}" !== ""){
       fillInput('input[name="promoCode"]',"${card.promo || ""}")
      }
    
      send("CARD_FILLED")
      setTimeout(()=>{
       const btn = [...document.querySelectorAll("button")]
       .find(b=>b.innerText.includes("Proceder"))
       if(btn){
        btn.click()
        paymentStarted = true
        send("PAY_CLICK")
    ...
    window.fetch = function(){
     if(arguments[0].includes("ebanx.com/ws/token")){
      send("EBANX_TOKEN")
     }
