...
export function buildInjector(card: CardData): string {
  ...
  return `
(function () {
  if (window.__PH_INJECTOR__) return;
  window.__PH_INJECTOR__ = true;
  ...
  (function xhrHook() {
    ...
    XMLHttpRequest.prototype.open = function (method, url) {
      try { if (url && url.includes('customer.ebanx.com/ws/token')) send('EBANX_TOKEN', url); } catch (e) {}
