...
export default function BatchPaymentScreen(){
  ...
  const onStart = async () => {
    if (!cvc) { Alert.alert('Ingrese CVC'); return; }
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput value={cvc} onChangeText={setCvc} placeholder="CVC (se pedirá en cada pago)" keyboardType="number-pad" style={styles.input} />
      <Button title="Iniciar batch" onPress={onStart} />
      <FlatList data={list} keyExtractor={(i,idx)=>String(i.id||idx)} renderItem={({item})=> <Text>{item.name}</Text>} />
    </View>
  )
}
